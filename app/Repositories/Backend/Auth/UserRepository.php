...
class UserRepository extends BaseRepository {
	 ...
	 * @return mixed
	 */
	public function getAllPaginated($paged = '', $orderBy = 'created_at', $sort = 'desc'): LengthAwarePaginator {
		return $this->model
	...
	public function create(array $data): User {
		return DB::transaction(function () use ($data) {
			$user = parent::create([
				...
				'gender' => isset($data['gender']) && $data['gender'] == '1' ? 1 : 2,
				'city' => $data['city'],
				'country' => $data['country'],
				'about' => $data['about'],
				'university' => $data['university'],
				'education_from_date' => date('Y-m-d H:i:s', strtotime($data['education_from_date'])),
				'education_to_date' => date('Y-m-d H:i:s', strtotime($data['education_to_date'])),
				'education_about' => $data['education_about'],
				'graduted' => isset($data['graduted']) && $data['graduted'] == '1' ? 1 : 0,
			...
			if ($user) {
				...
				if (is_array($data["user_interests"])) {
					foreach ($data["user_interests"] as $uik) {
					...
					$interest = new Interest();
					$interest->user_id = $user->id;
					$interest->title = $data["user_interests"];
					$interest->save();
				}
...
//				$deleted = Interest::where('user_id', $user->id)->delete();
